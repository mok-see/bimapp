...
def download_csv():
    session["dl_link"] = pandashelper.get_csv_table_download_link(session.file_name,session.DataFrame)

def download_excel():
    pandashelper.download_excel(session.file_name,session.DataFrame)

def download_filtered_csv():
    #pandashelper.download_csv(session.file_name,session["filtered_frame"])
    session["dl_link"] = pandashelper.get_csv_table_download_link(session.file_name,session["filtered_frame"])

def download_filtered_excel():
    pandashelper.download_excel(session.file_name,session["filtered_frame"])
...
def execute():  
    ...
    st.header(" 🧮 Model Properties and Quantities")
    if not "IsDataFrameLoaded" in session:
        initialize_session_state()
    if not session.IsDataFrameLoaded:
        load_data()
    if session.IsDataFrameLoaded:    
        tab1, tab2, tab3 = st.tabs(["Dataframe Utilities", "Properties Review" ,"Quantities Review"])
        with tab1:
            ## DATAFRAME REVIEW            
            ...
            if session.download_csv:
                        st.markdown(session["dl_link"], unsafe_allow_html=True)
            st.button("Download Excel", key="download_excel", on_click=download_excel)
        with tab2:
                if session.IsDataFrameLoaded:
                    type_selector = st.selectbox("Select Class", session.Classes, key="type_selector")
                    session["filtered_frame"] = pandashelper.filter_dataframe_per_class(session.DataFrame, session.type_selector)

                    st.write(session["filtered_frame"])
                    st.button("Download CSV", key="download_filtered_csv", on_click=download_filtered_csv)
                    if session.download_filtered_csv:
                        st.markdown(session["dl_link"], unsafe_allow_html=True)
                    st.button("Download Excel", key="download_filtered_excel", on_click=download_filtered_excel)
